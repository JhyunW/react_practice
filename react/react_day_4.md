...
## .jsx컴포넌트 정리
    ...
    - 컴포넌트 열림과 닫힘 텍스트 사이에 무언가를 전달하면 리액트가 출력할 위치를 모르기에 내용이 출력되지 않음. => 함수(props) 식으로 받는 인자를 입력한 후 {props.children} 식으로 사용가능
    또는 함수(children) 후 {children}으로 사용가능
    - on관련 props들이 많다 => onClick도 그중 하나
